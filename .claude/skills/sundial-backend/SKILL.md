...
| `check:attendance-recalc`                       | 寫入 `attendance_records` 的呼叫沒有在同一函式、同一個交易 handle 底下呼叫 `recalculateAttendanceResultForWorkDay`（出勤計畫 §4.3.1）。Stage 4 曾經真的漏掉 `create`（打卡）這條路徑，寫入成功但判定結果永遠不出現，且完全不會報錯——這支腳本把「記得抄既有形狀」換成掃描器擋，判準與 `check:audit-transaction` 的「同一個交易 handle」同一套思路 |
