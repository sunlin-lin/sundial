...
> **「來源自己宣告適用年度」算明示，這一類要寫。** 本條原本只反駁「拿下一版開始日的前一天」，沒有處理年度表——而 `9` 扣繳稅額表的「115年度」四個字就是政府明示的適用範圍，不是我們推導出來的。
>
> 不寫的後果實測過：`9` 有兩個年度缺口（108、110 沒有可用資源）。若不寫訖日，補算民國 110 年度的薪資會拿到 109 年度那一版的稅額——**一個完全合理的錯稅額**。寫了之後那兩個日期回「查無適用版本」，是明確的錯誤。
>
> 判準：訖日必須來自來源本身的宣告，不得由「下一版是哪天開始」算出來。實作上把它做成**必填參數**，逼每個來源回答「你有沒有明示的訖日」。
...
**`9` 扣繳稅額表的來源要用列表頁，不要用 data.gov.tw。** 年度在**連結的 `title`**（`財政部臺北國稅局薪資所得扣繳稅額表_115年度.csv`），**不是連結文字**——連結文字只有 `115`，而同一頁上「扣免繳申報收件統計表」那一項的連結文字也是 `107`、`111`。拿連結文字識別版本等於用**版面**而不是用資源本身。

**實際可用的是 7 個年度，不是 8 個**：民國 108 年度那一份的 `title` 裡沒有年度（就是 `薪資所得扣繳稅額表[CSV]`），而且它在政府端已經下架。判準以**名稱**為準——用「下載得到嗎」當候選判準的話，「政府哪天把某一年搬走」會變成看不見的缺口。

開放平臺的資料集（`25627`）只有當年度一份，且 title／description／資源名稱／`coverageStartedDate` **四處都沒有年度**——單靠它推不出生效日。但財政部臺北國稅局的 Open Data 下載專區把**每個年度各放一份 CSV、各有固定網址**，年度寫在連結文字裡（`…薪資所得扣繳稅額表_115年度.csv`），涵蓋 107–115（110 缺）。開放平臺目前指向的正是其中的 115 年度那一份，可交叉驗證。**副作用是歷史版本可以一次回補。**

**`8` 最低工資：走勞動部公告頁，而且只爬「歷年最低工資／基本工資調整」那一頁。**

放棄行政院公報 XML 的理由不是它比較難，是**它的失敗方向是錯的**：公報的金額寫成中文數字（「二萬九千五百元」），讀錯一個位就是 2,950 或 29,050——兩者都通得過 decimal 驗證、通得過形狀驗證，而錯的是全國法定工資下限。這個模組每一條規則都在避免「合理的錯值」，那條路是主動引進一個。次要理由：公報是全政府的，要靠關鍵字搜尋撈，而搜尋條件本身就是會靜靜漏掉新公告的地方。

**113 年以前那一頁（「基本工資之制訂與調整經過」）刻意不爬**，因此涵蓋範圍是**民國 114 年 1 月起**。原因不是句型變了（那可解），是**那一頁的公告有一半不是自足的**：

```
民國101年10月16日發布，自102年1月1日起實施，每小時基本工資調整為109元。   ← 只調時薪
民國102年4月2日發布，…每月基本工資調整為19,047元。                      ← 只調月薪
```

本資料集一個版本必須同時有月薪與時薪兩筆。單值公告只有兩條路：排除它 → `resolve` 那段期間回上一版，時薪是錯的**且完全合理**；或讓新版本「繼承」另一個值 → 那是 §7.2 禁止的推測。兩條都不能走，所以整頁不爬，民國 113 年以前查不到版本——**明確的錯誤，不是安靜的錯值**。
