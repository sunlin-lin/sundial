...
| 2 | 全民健康保險投保金額分級表 | data.gov.tw `20251`（16 個歷史版本） | 資源名稱「115年1月…」 |
| 3 | 勞工退休金月提繳工資分級表 | data.gov.tw `6274` ＋ 歷年版 `13335` | **資料欄位** `生效日` |
...
**歷史深度不一致，這會影響補算舊期薪資：** `2`（16 版）、`5`（19 版，回溯到民國 100 年）、`3`（歷年版 `13335` 每列都帶 `生效日`）、`9`（107–115）都能一次回補；但 `1`、`4`、`6` 只有**當期**版本（勞保的歷史在另一個資料集，日期是中文自由字串如「45年6月至49年1月」）。意思是勞保這幾類的歷史**只能從我們開始同步那天起累積**——補算更早的期間時會查不到版本，而依 §7.2 那會是明確的錯誤，不會靜靜算錯。
