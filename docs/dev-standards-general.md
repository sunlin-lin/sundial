...
### 7.6 全稱規則的定義域必須可判定，補集必須可枚舉

- **寫「所有 X 一律 Y，沒有例外」之前必須回答兩件事：** ⚠️（加規則的 PR 須在描述逐項回答）
  1. **X 的判準是否機械可算**——由路徑、檔名、schema、目錄結構推導得出，**不能是一個未定義的名詞**（「參數」「端點」「查詢」「共用」這類詞在不同人腦中指的不是同一組東西）；
  2. **X 的補集現在有幾個東西，且必須由同一支檢查列出來**——「補集為空」是一個**要被驗證的斷言**，不是一個假設。
- **理由：兩條各自「沒有例外」的規則，若定義域重疊而處置相反，實作者只能違反其中一條，而規範上看不出來哪一條該讓。** 這種死結**沒有任何檢查抓得到**，因為每條檢查單獨看都是綠的——實作者一定會去繞開會紅的那條，改違反靠 review 的那條，於是**規範的 bug 會表現為一份看起來合規的程式碼**。等到有人發現，那份程式碼已經被當成範例抄了好幾份。
- **推論：「照規範寫不出來」的 PR 一律是規範的 bug，不是實作者的問題。** 撞到時的正確動作是**停下來改規範**，不是挑一條比較不痛的來違反。挑一條違反的代價是：那個決定不會被記錄在任何地方，下一個實作者會重新撞一次同一面牆，並且**可能挑另一條來違反**——於是同一個死結在系統裡長出兩種互相矛盾的解法。

### 7.7 不變式規則必須指名 base case 的產出者

- **任何形如「A 只能來自 B」的規則，必須在同一節指名三件事：第一個 B 由誰產生、在哪一層產生、由哪一條檢查證明只有那一個地方能產生。** ⚠️（加規則的 PR 須在描述指出）
- **理由：沒有指名 base case 的不變式，在實作上是循環定義**——它保護穩態，卻讓系統**無法離開初始狀態**。「所有查詢都要帶公司範圍，而公司範圍只能來自已驗證的身分」在穩態下完全正確，但「第一次的公司範圍從哪來」沒有答案，於是登入這支端點照規範寫不出來。
- **而繞過的那一處，通常正是安全性最關鍵的地方。** 這不是巧合：會出現這種不變式的地方，本來就是識別、授權、隔離這類最需要保護的邊界；實作者被迫在那裡開一個規範沒有描述的口子，**而規範沒有描述它，就沒有任何檢查看得見它**。指名 base case 的產出者，就是把那個口子從「某人私下開的」變成「規範指定的、只有一處、有檢查證明只有一處」。
