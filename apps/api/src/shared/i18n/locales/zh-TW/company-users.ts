 ...
 * ——這一段就是目錄名，不是另外取的領域名。
 *
 * ⚠️ **本檔只有「字」，而且有三則是刻意含糊的**（§3.2）：`main.errors.username-taken`
 * 不透露這個帳號屬於哪一家公司；`main.errors.company-user-not-found` 與
 * `roles.errors.company-user-not-found` 都是「查無此成員」與「這位成員屬於別家公司」共用同一句。
 * **理由分別寫在 `modules/company-users/main/company-users-main.errors.ts` 與
