 ...
 * 任一失敗時整筆取消」。本函式依序呼叫六個被編排模組的 `*InTransaction` 動作
 ...
 * ## 六步的呼叫順序，以及為什麼是這個順序
 *
 * 1. `employees.main.create` —— 沒有它，後面五步全部沒有 `employeeId` 可用。
 * 2. `employments.main.create` —— 需要剛建立的 `employeeId`；沒有任職就沒有 `employmentId`，
 *    第 3 步（部門歸屬）掛在任職底下，不能先做。
 * 3. `employments.department-histories.create` —— 需要 `employmentId`。
 * 4. `withholding.main.create` —— 需要 `employeeId`（扣繳掛在員工底下，不是任職底下，字典原文）。
 * 5. `company-users.main.create`（新增登入帳號並加入公司）—— 需要 `employeeId`。
 * 6. `company-users.roles.create`（指派角色）—— 需要上一步產生的 `companyUserId`。
 *
 * 4／5 兩步彼此沒有依賴，順序對調也一樣正確；固定成「先扣繳、後帳號」單純是為了跟 UI
 * 表單分節的順序（`docs/ui/20-employee-list.md` §2.3 在 §2.4 之前）一致，方便對照。
 *
 * ## 每一步各自負責自己那一筆稽核
 *
 * 六個被呼叫的 `*InTransaction` 動作**各自在自己的實作裡呼叫 `recordAudit(tx, ...)`**
 ...
 * create`，共六筆），本函式**不需要、也不應該**再另外呼叫一次——那會讓同一項異動被記錄兩遍。
 * 「稽核與業務寫入同一交易」這條規則因此對本函式自動成立：六筆稽核與六筆業務寫入共用同一個
...
export const createOnboardingInTransaction = async (
  ...
  // ---- 6. 角色指派（至少一筆，由 request schema 保證） ----
