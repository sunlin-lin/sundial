 ...
 *
 * 與 `find.repository.ts` 的差別只在最後一步的映射：那一支**當場遮罩**（明文一步都不往
 * 上層走，§5.1），本支**回明文**——唯一合法的呼叫者是稽核（透過 `buildAuditChanges`），
 * 理由與 `toPlaintextProfile` 的檔頭相同：`presence` 級的變更判定必須基於明文比對。
 *
 ...
 * 什麼，反而多一層要維護的間接。真正共用的是零 IO 的映射（`toPlaintextProfile`），已經在
...
export const findEmployeeAuditSnapshot = async (
  ...
  return row === undefined ? null : toPlaintextProfile(row)
}
