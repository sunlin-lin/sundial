/**
 * 扣繳設定的端點測試（§7.1）。形狀比照 `departments/main/__tests__/departments-main.endpoints.test.ts`。
 */
...
import { withholdingMainRoutes } from '../withholding-main.routes.ts'
...
const accessControl: AccessControlPorts = {
  ...
  loadPermissionCodes: () => Promise.resolve(new Set(['withholding.main.list', 'withholding.main.create'])),
}
  ...
  new Elysia()
    ...
    .use(
      new Elysia({ name: 'test-authenticated-group' })
        ...
        .use(withholdingMainRoutes({ db, clock })),
    )
...
describe('withholding/main endpoints (integration)', () => {
  test('新增扣繳設定成功，可由 list 讀回', async () => {
    const { token, employeeId } = await registerCompanyWithEmployee()

    const created = await call<WithholdingDetailShape>('/withholding/main/create', token, {
      employeeId,
      withholdingMethodCode: 1,
      effectiveFrom: '2024-01-01',
    })
    expect(created.status).toBe(200)
    expect(created.payload.data.withholdingMethodCode).toBe(1)
    expect(created.payload.data.effectiveTo).toBeNull()

    const listed = await call<{ data: WithholdingDetailShape[] }>('/withholding/main/list', token, {
      employeeId,
      perPage: 20,
      currentPage: 1,
    })
  ...
  test('重疊的扣繳期間回 422 period-overlap；不重疊的可以再新增一筆', async () => {
    const { token, employeeId } = await registerCompanyWithEmployee()

    const first = await call<WithholdingDetailShape>('/withholding/main/create', token, {
      employeeId,
      withholdingMethodCode: 1,
      effectiveFrom: '2024-01-01',
    ...
    const overlapping = await call('/withholding/main/create', token, {
      employeeId,
      withholdingMethodCode: 2,
      effectiveFrom: '2024-03-01',
    })
    expect(overlapping.status).toBe(422)
    expect(overlapping.payload.errors.map((error) => error.code)).toEqual(['withholding.main.errors.period-overlap'])

    const nonOverlapping = await call<WithholdingDetailShape>('/withholding/main/create', token, {
      employeeId,
      withholdingMethodCode: 2,
  ...
  test('目標員工不存在時回 422 employee-not-found', async () => {
    ...
    const result = await call('/withholding/main/create', token, {
      employeeId: crypto.randomUUID(),
      withholdingMethodCode: 1,
      effectiveFrom: '2024-01-01',
    })
    expect(result.status).toBe(422)
    expect(result.payload.errors.map((error) => error.code)).toEqual(['withholding.main.errors.employee-not-found'])
  })
})
