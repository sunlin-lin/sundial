...
describe('toAttendanceDailyRecordListQuery：只送後端認得的條件（見檔頭：狀態篩選是後端缺口）', () => {
  test('部門與人員都未篩選時，query 裡不含這兩個鍵（不是設成 undefined）', () => {
    const query = toAttendanceDailyRecordListQuery({ date: '2026-08-28', departmentId: null, employeeId: null }, 1)
  ...
  test('有篩選部門與人員時帶入對應的鍵', () => {
    const query = toAttendanceDailyRecordListQuery(
      { date: '2026-08-28', departmentId: 'dept-1', employeeId: 'employee-1' },
      2,
    )
    expect(query).toEqual({
      date: '2026-08-28',
