 ...
 * 這件事目前完全由後端把關（無權限回 `901`，本頁顯示無權限而不導登入頁），缺口見 `.route.ts`。
 *
 * 呈現決策一律不在這裡：狀態的文字與色彩、失敗原因的空值處置、筆數與總數的換算、回聲比對，
 * 全部在 `.view.ts` 與 `.dataset.view.ts`（§1.3）；查詢的組裝在 `.payload.ts`。
 */
...
import { DATASET_OPTIONS, DEFAULT_DATASET_CODE } from './regulatory-sync.dataset.view.ts'
import {
  SYNC_LIST_PER_PAGE,
  toSyncListQuery,
...
import { toDisplayRows, type SyncLogDisplayRow } from './regulatory-sync.view.ts'
...
/** 登出。與 dashboard 同一段：`AppShell` 刻意不碰 store 與 API，登出後要去哪裡是頁面的事。 */
...
const load = (): void => {
  ...
  regulatorySyncList(query)
    .then((page) => {
      ...
      if (!isSyncListEcho(page, query)) return

      rows.value = toDisplayRows(page.data, $t)
      totalCount.value = toTotalCount(page.pagination.totalCount)
...
<template>
  ...
  >
    ...
    <div v-else-if="failure !== null">
      ...
      <ElButton class="mt-4" :loading="isLoading" @click="load">
    ...
    <div v-else>
      ...
      <!--
        ...
        `layout` 刻意**不含 `total`**：那一段是 Element Plus 自己的文案（`Total 5`），
        而全站還沒有設定它的語系（`ElConfigProvider` 應該掛在 `App.vue`／`main.ts`，
        不在本頁的檔案範圍內）。加上去會在一個 zh-TW 的畫面上出現一個英文字，
        而那不是語系檔管得到的字串。語系設好之後把 `total` 加回來即可。
      -->
      <ElPagination
